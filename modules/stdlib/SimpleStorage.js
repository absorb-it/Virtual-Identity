...
function SimpleStorageCps(aTblName) {
  ...
  this.dbConnection = Services.storage.openDatabase(FileUtils.getFile(KEY_PROFILEDIR, [FILE_SIMPLE_STORAGE]));
...
SimpleStorageCps.prototype = {
  ...
  set: function _SimpleStorage_set(aKey, aValue, k) {
    this.has(aKey, (function (aResult) {
      let query = aResult ? "UPDATE #1 SET value = :value WHERE key = :key" : "INSERT INTO #1 (key, value) VALUES (:key, :value)";
      let statement = this.dbConnection.createStatement(query.replace("#1", this.tableName));
      statement.params.key = aKey;
      statement.params.value = JSON.stringify({
        value: aValue
      });
      statement.executeAsync({
        handleResult: function (aResultSet) {},
