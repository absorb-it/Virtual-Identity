...
function replyAllParams(aIdentity, aMsgHdr, k) {
  ...
  let finish = function (to, cc, bcc) {
    ...
    hashMap[email] = null;
    cc = cc.filter(function ([name, email]) let (r = (email in hashMap))
      (hashMap[email] = null, !r)
    );
    bcc = bcc.filter(function ([name, email]) let (r = (email in hashMap))
      (hashMap[email] = null, !r)
    );
    k({
      to: to,
