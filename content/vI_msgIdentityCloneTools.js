...
var msgIdentityCloneTools = {	
	...
	signatureSwitch: function(existingIdentity) {
		...
		if (!existingIdentity) {
			...
			try { if (vI.vIprefs.get("hide_signature") && ss_signature.length == 0) {
			...
			try { if (vI.vIprefs.get("hide_sMime_messageSignature")) {
			...
			try { if (vI.vIprefs.get("hide_openPGP_messageSignature")) {
	...
	initReplyTo : function() {
		if (vI.vIprefs.get("autoReplyToSelf")) {
