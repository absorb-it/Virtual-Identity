    ...
    var smartIdentity = {
      ...
      init: function (currentWindow) {
        ...
        if (gMsgCompose === null) { // this should not happen - but I faced this issue. Than try to use the one stored globally
          Log.debug("######## strange, gMsgCompose is not set. Try to use previously stored one...");
        ...
        case msgComposeTypeReference.ReplyToList:
          ...
          smartIdentity._smartIdentityCollection = new vI.smartIdentityCollection(msgHdr, getCurrentIdentity(), document.getElementById("virtualIdentityExtension_msgIdentityClone").vid, newsgroup, this._getRecipients(currentWindow));
      ...
      _getRecipients: function (currentWindow) {
        var recipients = [];
        for (var row = 1; row <= top.MAX_RECIPIENTS; row++) {
          ...
          if (recipientType == "addr_reply" || recipientType == "addr_followup" ||
            vI.storage.isDoBcc(row, currentWindow) || currentWindow.awGetInputElement(row).value.match(/^\s*$/)) continue;
          recipients.push({
            recipient: currentWindow.awGetInputElement(row).value,
      ...
      __smartIdentitySelection: function (autocreate) {
        ...
        if (!autocreate && vI.vIprefs.get("idSelection_ask") &&
          ...
          window.openDialog("chrome://v_identity/content/vI_smartReplyDialog.xul", 0,
            ...
            /* callback: */
            smartIdentity.changeIdentityToSmartIdentity).focus();
      ...
      changeIdentityToSmartIdentity: function (allIdentities, selectedValue) {
        ...
        if (document.getElementById("virtualIdentityExtension_msgIdentityClone").vid) {
          ...
          if (allIdentities.number > 1) label += " " + smartIdentity.stringBundle.GetStringFromName("vident.smartIdentity.moreThanOne");
