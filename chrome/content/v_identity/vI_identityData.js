...
function identityData(email, fullName, id, smtp, extras, sideDescription, existingID) {
	...
	this.extras = extras?extras:new vI.storageExtras();
...
// 			vI.notificationBar.dump("## identityData: parseEmail _fullName = '" + this._fullName + "'\n");
// 			vI.notificationBar.dump("## identityData: parseEmail _email =    '" + this._email + "'\n");
		}
...
// 		vI.notificationBar.dump("## identityData: cleanName init '" + fullName + "'\n");
		var _fullName = fullName.replace(/^\s+|\s+$/g,"");
		if (_fullName.search(/^\".+\"$|^'.+'$/g) != -1) {
			_fullName = this.cleanName(_fullName.replace(/^\"(.+)\"$|^'(.+)'$/g,"$1$2"));
		}
// 		vI.notificationBar.dump("## identityData: cleanName done '" + _fullName + "'\n");
		return _fullName;
	},
	...
	isExistingIdentity : function(ignoreFullNameWhileComparing) {
		vI.notificationBar.dump("## identityData: isExistingIdentity: ignoreFullNameWhileComparing='" + ignoreFullNameWhileComparing + "'\n");
// 		vI.notificationBar.dump("## identityData base: fullName.toLowerCase()='" + this.fullName + "' email.toLowerCase()='" + this.email + "' smtp='" + this.smtp.key + "'\n");
...
// 				vI.notificationBar.dump("## identityData comp: fullName.toLowerCase()='" + identities[j].fullName.toLowerCase() + "' email.toLowerCase()='" + identities[j].email.toLowerCase() + "' smtp='" + identities[j].smtpServerKey + "'\n");
				...
				if (	(email.toLowerCase() == idEmail.toLowerCase()) &&
					this.smtp.equal(new smtpObj(identities[j].smtpServerKey))	) {
						...
						if ( this.fullName.toLowerCase() == identities[j].fullName.toLowerCase() ) {
							vI.notificationBar.dump("## identityData: isExistingIdentity: " + this.combinedName + " found, id='" + identities[j].key + "'\n");
		...
		if ( ignoreFullNameWhileComparing && ignoreFullNameMatchKey ) {
			vI.notificationBar.dump("## identityData: isExistingIdentity: " + this.combinedName + " found, id='" + ignoreFullNameMatchKey + "'\n");
			return 	ignoreFullNameMatchKey;
		}

		vI.notificationBar.dump("## identityData: isExistingIdentity: " + this.combinedName + " not found\n");
		return null;
	},
	...
	getCompareMatrix : function() {
		...
		var string = "";		
		var saveBaseId = (vI.statusmenu.objSaveBaseIDMenuItem.getAttribute("checked") == "true")
		var saveSMTP = (vI.statusmenu.objSaveSMTPMenuItem.getAttribute("checked") == "true")
...
identityCollection.prototype =
{
	...
	dropIdentity : function(index) {
		vI.notificationBar.dump("## identityCollection:   dropping address from inputList: " + this.identityDataCollection[index].combinedName + "\n");
		...
		for (var index = 0; index < this.number; index++) {
			if (this.identityDataCollection[index].email == identityData.email &&
				...
				if (this.identityDataCollection[index].fullName == "" && identityData.fullName != "") {
					...
					vI.notificationBar.dump("## identityCollection:   added fullName '" + identityData.fullName
				...
				if (!this.identityDataCollection[index].id.key && identityData.id.key) {
					...
					vI.notificationBar.dump("## identityCollection:   added id '" + identityData.id.value
						+ "' smtp '" + identityData.smtp.value + "' (+extras) to stored email '" + this.identityDataCollection[index].email +"'\n")
				}
				return;
			}
		}
		vI.notificationBar.dump("## identityCollection:   add new address to result: " + identityData.combinedName + "\n")
